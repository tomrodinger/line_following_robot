 ...
 * @return  0 on success, negative errno code on fail.
 */
static int cdc_acm_class_request_handler_idx0(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    switch (setup->bRequest) {
        case CDC_REQUEST_SET_LINE_CODING:
            ...
            memcpy(&usbd_cdc_acm_cfg.line_coding, *data, sizeof(usbd_cdc_acm_cfg.line_coding));
            usbd_cdc_acm_set_line_coding_idx(0, usbd_cdc_acm_cfg.line_coding.dwDTERate, usbd_cdc_acm_cfg.line_coding.bDataBits,
                                         usbd_cdc_acm_cfg.line_coding.bParityType, usbd_cdc_acm_cfg.line_coding.bCharFormat);
            break;
 ...
 * @return  0 on success, negative errno code on fail.
 */
static int cdc_acm_class_request_handler_idx1(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    switch (setup->bRequest) {
        case CDC_REQUEST_SET_LINE_CODING:
            ...
            memcpy(&usbd_cdc_acm_cfg.line_coding, *data, sizeof(usbd_cdc_acm_cfg.line_coding));
            usbd_cdc_acm_set_line_coding_idx(1, usbd_cdc_acm_cfg.line_coding.dwDTERate, usbd_cdc_acm_cfg.line_coding.bDataBits,
                                         usbd_cdc_acm_cfg.line_coding.bParityType, usbd_cdc_acm_cfg.line_coding.bCharFormat);
            break;
...
__weak void usbd_cdc_acm_set_line_coding_idx(uint8_t idx, uint32_t baudrate, uint8_t databits, uint8_t parity, uint8_t stopbits)
{
}

void usbd_cdc_add_acm_interface_idx(usbd_class_t *class, usbd_interface_t *intf, uint8_t idx)
{
    static usbd_class_t *last_class = NULL;
    ...
    if (idx == 0) {
        intf->class_handler = cdc_acm_class_request_handler_idx0;
    } else {
        intf->class_handler = cdc_acm_class_request_handler_idx1;
    }
